...
if (debug) System.err.println(indent()+"composite="+composite+" clientArea="+clientArea);
...
if (debug) System.err.println(indent()+"fixedWidth="+fixedWidth+" fixedHeight="+fixedHeight);
    ...
    height       = clientArea.height-marginTop-marginBottom;
...
if (debug) { System.err.print(indent()+"row sizes: ");for (int i = 0; i < rows;    i++) System.err.print(" "+rowSizes[i]   +(rowExpandFlags[i]   ?"*":""));System.err.println(); }
    width        = clientArea.width-marginLeft-marginRight;
...
if (debug) System.err.println(indent()+"layout:");
    for (int i = 0; i < children.length; i++)
    {
      ...
      int childX = clientArea.x+marginLeft+columnSizesSum[tableLayoutData.column]+tableLayoutData.padX;
      int childY = clientArea.y+marginTop +rowSizesSum   [tableLayoutData.row   ]+tableLayoutData.padY;
...
if (debug) System.err.println(indent()+String.format("Fixed column width: %d",fixedColumnWidth));
        ...
        for (int i = 0; i < tableColumns.length; i++)
        {
          ...
          {
            width = (int)((clientArea.width-fixedColumnWidth)*columnWeights[i]);
